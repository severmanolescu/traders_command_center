"""
Crypto Market Global Data Module
This module fetches global cryptocurrency market data from the CoinMarketCap API
...
def get_dict_crypto_market_global_data():
    ...
    Returns:
        dict: A dictionary containing global market data including market cap,
        volume, and dominance metrics.
    ...
    return {
        "market_cap_value": format_market_cap(
            response["data"]["quote"]["USD"]["total_market_cap"]
        ),
        "market_cap_change": round(
            response["data"]["quote"]["USD"][
                "total_market_cap_yesterday_percentage_change"
            ],
            2,
        ),
        "market_cap_24h_change": format_market_cap(
            response["data"]["quote"]["USD"]["total_volume_24h"]
        ),
        "btc_dominance": str(round(response["data"]["btc_dominance"], 2)) + "%",
        "eth_dominance": str(round(response["data"]["eth_dominance"], 2)) + "%",
        "defi_market_cap": format_market_cap(
            response["data"]["quote"]["USD"]["defi_market_cap"]
        ),
        "defi_24h_change": format_market_cap(
            response["data"]["quote"]["USD"]["defi_volume_24h"]
        ),
    }
