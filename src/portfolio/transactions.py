"""
Load and manage cryptocurrency transactions.
"""
...
def create_csv_content(symbol):
    ...
    try:
        ...
        if symbol.lower() == "all":
            ...
            filename = (
                f"all_transactions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
        else:
            transactions = load_transactions_by_symbol(symbol)
            filename = (
                f"{symbol}_transactions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
    ...
    # pylint: disable=broad-exception-caught
        ...
        logger.error("Error creating CSV content: %s", str(e))
...
def update_buy(
    symbol, amount, price, date=None, exchange=None, wallet=None, notes=None
):
    """
